...
# Build streamlit_app.py from compact code segments

p1 = """import streamlit as st, cv2, numpy as np, tempfile, os, math, pandas as pd
...
def process_frame(pr,fr,sel,sk_frames,frame_idx):
...
p3 = """inp=st.radio('Input',['URL (up to 4GB)','Upload (up to 200MB)'],horizontal=True)
      ...
      if st.button('Start Professional Analysis'):
        ...
        with st.spinner('Analyzing frames...'):
          while True:
            ...
            if not ret: break
            ofr=fr.copy()
            _,findings,ball_pos=process_frame(pr,fr,sel,sk,frames)
            if findings: all_findings.extend(findings);last_fb=findings[0].get('problem','')+' - '+findings[0].get('corrective_drills','')
            # Draw overlays
            pr.hm.overlay(fr)  # call overlay to compute but we draw custom below
...
p4 = """        fiq_st=pr.fiq.stats(sel);ra_st=pr.ra.get_stats(sel)
